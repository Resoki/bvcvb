...
module.exports = {
  ...
  async execute(client, message, args, color) {
    if (
      ...
      if (args[0] === "all") {
        ...
        message.channel.send(
          `${message.guild.channels.cache.size} salons fermés`
        );

        const embed = new Discord.MessageEmbed()
          .setDescription(
            `:lock: | ${message.author.tag} vient de fermé tous les salons du serveur \n Executeur : <@${message.author.id}>`
          )
          .setTimestamp()
          .setColor(color)
          .setFooter({ text: `📚` });
    ...
    if (
      ...
      message.channel.send(
        `Les membres ne peuvent plus parler dans <#${channel.id}>`
      );
    }
    ...
    const embed = new Discord.MessageEmbed()
      .setColor(color)
      .setDescription(
        `<@${message.author.id}> a \`vérouiller\` le salon <#${message.channel.id}>`
      )
      .setTimestamp()
      .setFooter({ text: `📚` });
    client.channels.cache
