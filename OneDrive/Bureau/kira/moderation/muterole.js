...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    if (
      ...
      let muterole =
        (await message.guild.roles.cache.get(muted)) ||
        ...
        message.channel.send({ embeds: [embed] }).then(async (mm) => {
          ...
          channels.forEach((channel) => {
            ...
            embed2.setDescription(
              `**__D'autres permission déjà existantes peuvent rendre innefficace le mute pour certains rôles dans les salons suivants :__**\n\n**${channel.name}**\n- ${muterole.name}\n`,
              true
            );
            embed2.setFooter({
              text: 'Tous les rôles ayant la permissons "envoyer des messages" en vert seront insensible au mute',
            });
          });
      ...
      if (!muterole) {
        ...
        embed.setColor(color);
        embed.setTitle(
          `<a:loading:957097853694664746> Création d'un rôle muet en cours`
        );
