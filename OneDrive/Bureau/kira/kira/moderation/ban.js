    ...
    if (
      ...
      if (!member) {
        return message.reply(
          "Merci de mentionner l'utilisateur que vous souhaitez bannir du serveur !"
        );
      }
      ...
      message
        .reply({ content: `${member} à été banni du serveur` })
        .catch((err) => err);
      member.send({
        content: `Tu as été banni par ${message.author} pour la raison suivante: \n\n ${reason}`,
      });
      ...
      const embed = new Discord.MessageEmbed()
        .setColor(color)
        .setDescription(
          `<@${message.author.id}> a \`banni\` ${member} du serveur\nRaison : ${reason}`
        )
        .setTimestamp()
        .setFooter({ text: `📚` });
      ...
      if (
        ...
        return message.reply(
          `Vous ne pouvez pas ban un membre au dessus de vous`
        );
      }
