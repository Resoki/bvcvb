    ...
    if (
      ...
      if (muteUser.voice.serverMute) {
        return message.channel.send(
          "Le membre n'est pas dans un salon vocal ou est déjà mute vocal."
        );
      }
      ...
      try {
        muteUser.user.send(
          `Vous avez été **Mute** sur **${message.guild.name}**, Raison: **${
            muteReason || "Aucune"
          }**.`
        );
      } catch (err) {
      ...
      message.channel.send(
        `**${
          muteUser.user.tag
        }** a été mute avec succès sur le serveur. Raison: **${
          muteReason || "Aucune"
        }**. `
      );
      ...
      const embed = new Discord.MessageEmbed()
        .setColor(color)
        .setDescription(
          `<@${message.author.id}> a \`voicemute\` ${muteUser}\nRaison: ${muteReason}`
        )
        .setTimestamp()
        .setFooter({ text: `📚` });
