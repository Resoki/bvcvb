  ...
  async execute(client, member) {
    ...
    if (lock.get(`serverlock_${member.guild.id}`) === "lock") {
      ...
      const embed = new Discord.MessageEmbed()
        .setColor(color)
        .setDescription(
          `${member} à été **kick** pour avoir \`rejoint pendant que le serveur était verrouillé\``
        );
      client.channels.cache
        .get(rlog.fetch(`${member.guild.id}.raidlog`))
        .send({ embeds: [embed] })
        .catch(console.error);
    }
    ...
    if (db.get(`blacklist.${member.id}`)) {
      member.send({
        content: `Vous etes blacklist de **${member.guild.name}** vous ne pouvez pas rejoindre le serveur`,
      });
      member.guild.members.ban(member.id, { reason: `Blacklist` });
      const embed = new Discord.MessageEmbed()
        .setColor(color)
        .setDescription(
          `${member} a rejoit en étant __blacklist__, il à été **ban**`
        );
      client.channels.cache
        .get(rlog.fetch(`${member.guild.id}.raidlog`))
        .send({ embeds: [embed] })
    ...
    if (member.user.bot) {
      if (atb.get(`config.${member.guild.id}.antibot`) === true) {
        ...
        let perm =
          config.app.owners == action.executor.id ||
          config.app.funny == action.executor.id ||
          owner.get(`owners.${action.executor.id}`);

        if (!perm) {
          member.kick("Antibot");

          if (punish.get(`sanction_${member.guild.id}`) === "ban") {
            member.guild.members.ban(action.executor.id, {
              reason: `Anti Bot`,
            });
            ...
            member.guild.members.kick(action.executor.id, {
              reason: `Anti bot`,
            });
          }
          ...
          const embed = new Discord.MessageEmbed()
            ...
            .setTimestamp();
          client.channels.cache
            .get(db.fetch(`${member.guild.id}.raidlog`))
            .send({ embeds: [embed] })
            .catch(console.error);
        }
      }
    ...
    if (member.user) {
      ...
      if (joinsettings === true) {
        ...
        const content = messagejoin
          ...
          .replaceAll(
            "{ServerLevel}",
            `${premiumTier[member.guild.premiumTier]}`
          )
          .replaceAll(
            "{VocalMembersCount}",
