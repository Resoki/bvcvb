...
module.exports = {
  ...
  async execute(client, oldState, newState) {
    if (db.fetch(`${newState.guild.id}.antivoc.${newState.channelId}`)) {
      if (
        owner.get(`owners.${newState.id}`) ||
        wl.get(`${newState.guild.id}.${newState.id}.vl`) ||
        (config.app.owners === newState.id) === true ||
        (client.user.id === newState.id) === true
      )
        return;
      newState.disconnect();
      const embed = new Discord.MessageEmbed()
        .setTitle(`Anti Join`)
        .setDescription(
          `L'anti join est activé dans le salon <#${newState.channel.id}>\nVous devez etre owner/wl/vl pour y accèder`
        )
    ...
    if (db.get(`tempvocsettings_${oldState.guild.id}`) === true) {
      ...
      if (oldState.channel === null || oldState.channel) {
        if (newState.channelId === salontempvoc) {
          ...
          oldState.guild.channels
            .create(`⏱️・${newState.member.user.username}`, {
              type: "GUILD_VOICE",
              parent: category,
            ...
            .then((funny) => {
              ...
              db.set(
                `tempvoc_${newState.guild.id}_${newState.member.id}`,
                funny.id
              );
            });
        }
      ...
      if (
        oldState.channel.id ==
        db.get(`tempvoc_${newState.guild.id}_${newState.member.id}`)
      ) {
