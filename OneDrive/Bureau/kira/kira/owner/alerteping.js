    ...
    if (
      ...
      if (!args[0]) {
        const embed = new Discord.MessageEmbed()
          ...
          .setDescription(
            `Quel role souhaitez vous mentionner lors d'un ajout de permissions administrateurs`
          )
          .setColor(color);
        ...
        message.channel
          .send({ embeds: [embed], components: [alerteping] })
          .then(async (msg) => {
            ...
            collector.on(`collect`, async (cld) => {
              ...
              if (cld.customId === "everyone") {
                ...
                message
                  .reply({
                    content: `**@Everyone** sera désormais ping à chaque ajout de permissions administrateurs`,
                    ephemeral: true,
                  })
                  .then((msg) => {
                ...
                message
                  .reply({
                    content: `**@Here** sera désormais ping à chaque ajout de permissions administrateurs`,
                    ephemeral: true,
                  })
                  .then((msg) => {
                    setTimeout(() => msg.delete(), 6000);
      ...
      if (args[0]) {
        ...
        if (!role)
          return message.channel.send({
            content: `Merci de spécifiez le rôle à ajouter`,
          });

        message.channel.send({
          content: `Le role ${role} sera désormais ping à chaque ajout de **permissions dangereuses**`,
        });
