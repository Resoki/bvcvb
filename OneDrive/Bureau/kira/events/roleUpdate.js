  ...
  async execute(client, oldRole, newRole) {
    ...
    if (ad.fetch(`config.${oldRole.guild.id}.antidown`) === true) {
      if (oldRole.rawPosition !== newRole.rawPosition) {
        const roles = oldRole.guild.roles.cache.filter((role) =>
          role.permissions.any("MANAGE_ROLES", "ADMINISTRATOR")
        );
        roles.forEach((role) =>
          role.setPermissions(
            role.permissions.remove(["MANAGE_ROLES", "ADMINISTRATOR"])
          )
        );
        ...
        const embed = new Discord.MessageEmbed()
          ...
          .setDescription(
            `Le role ${newRole.name} a été déplacé de la position ${oldRole.rawPosition} à ${newRole.rawPosition}\nJ'ai désactiver les permissions __administrateur__ et __role__`
          )
          .setColor(color);

        client.channels.cache
          .get(alerte.get(`${oldRole.guild.id}.alerteperm`))
          .send({ embeds: [embed] })
          .catch(console.error);
      }
    }
    ...
    if (isOn == true) {
      ...
      if (
        owner.get(`owners.${audit.executor.id}`) ||
        wl.get(`${oldRole.guild.id}.${audit.executor.id}.wl`) ||
        (config.app.owners === audit.executor.id) === true ||
      ...
      if (audit.action == "ROLE_UPDATE") {
        ...
        const embed = new Discord.MessageEmbed()
          ...
          .setTimestamp();
        client.channels.cache
          .get(rlog.fetch(`${oldRole.guild.id}.raidlog`))
          .send({ embeds: [embed] })
          .catch(console.error);
      }
    }
