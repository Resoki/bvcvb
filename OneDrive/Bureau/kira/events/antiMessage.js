...
module.exports = {
  ...
  async execute(client, message, channel) {
    ...
    let antilinkinvite = await al.get(
      `config.${message.guild.id}.antilinkinvite`
    );
    ...
    if (
      owner.get(`owners.${message.member.id}`) ||
      wl.get(`${message.guild.id}.${message.member.id}.wl`) ||
      (config.app.owners === message.author.id) === true
    )
      return;
    ...
    if (antilinkinvite == true) {
      if (isLink == true) {
        ...
        client.channels.cache
          .get(rlog.fetch(`${message.guild.id}.raidlog`))
          .send({ embeds: [embed] })
          .catch(console.error);
      }
    }
