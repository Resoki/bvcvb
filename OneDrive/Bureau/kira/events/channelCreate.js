...
module.exports = {
  ...
  async execute(client, channel) {
    let muterole =
      (await channel.guild.roles.cache.get(
        db.fetch(`muterole_${channel.guild.id}`)
      )) ||
      channel.guild.roles.cache.find((role) => role.name === `muet`) ||
      channel.guild.roles.cache.find((role) => role.name === `Muted`) ||
      channel.guild.roles.cache.find((role) => role.name === `Mute`);
    if (muterole) {
    ...
    if (atc.get(`config.${channel.guild.id}.antichannelcreate`) == true) {
      ...
      if (
        ...
        wl.get(`${channel.guild.id}.${audit.executor.id}.wl`) ||
      ...
      client.channels.cache
        .get(rlog.fetch(`${channel.guild.id}.raidlog`))
        .send({ embeds: [embed] })
        .catch(console.error);
    }
  },
