    ...
    if (
      ...
      if (isNaN(guildID) || !guildID || guildID.length != 18) {
        return message.channel.send(
          `> Vous devez indiquer l'id du serveur à quitter.`
        );
      } else {
        const guild = client.guilds.cache.get(guildID);
        if (guild === undefined)
          return message.channel.send(`> Votre bot n'est pas sur ce serveur.`);
        if (!guild.available)
          return message.channel.send(
            "> Serveur non disponible, réessayez plus tard."
          );
        ...
        client.guilds.cache
          ...
          .then((x) => {
            message.channel
              .send(`> J'ai bien quitté le serveur ${x.name}`)
              .catch(() => {});
          })
          .catch((err) => {
            console.log(
