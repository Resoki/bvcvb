    ...
    if (
      ...
      if (
        ...
        return message.reply(
          "> Le Menu n'a pas pu etre crée car le rôle séléctionné contient des permissions **Dangereuses**"
        );
      }
      ...
      const rolemenu = new Discord.MessageActionRow().addComponents(
      ...
      collector.on("collect", async (c) => {
        ...
        if (value === "roles") {
          if (!c.member.roles.cache.has(role.id)) {
            c.reply({
              content: `${c.user}, je vous ai donné le rôle \`${role.name}\``,
              ephemeral: true,
            });
            c.member.roles.add(role).catch((err) => console.log(err));
          } else {
            c.reply({
              content: `${c.user}, je vous ai enlevé le rôle \`${role.name}\``,
              ephemeral: true,
            });
