      ...
      try {
        ...
        async function first_layer() {
          ...
          let MenuEmbed = new MessageEmbed()
            ...
            .setTitle("Soutien")
            .setImage(
              "https://cdn.discordapp.com/attachments/904084986536276059/991542738879266836/standard_3.gif"
            )
            .setDescription(
              `**Choississez une option pour configurer le rôle soutien et son statut**`
            );
          let used1 = false;

          const menumsg = await message.channel.send({
            embeds: [MenuEmbed],
            components: [new MessageActionRow().addComponents([menuoptions])],
          });
          ...
          const antichannel = new MessageEmbed()
            ...
            .setColor(color)
            .setImage(
              "https://cdn.discordapp.com/attachments/904084986536276059/991543407694585866/role.gif"
            );

          const antichanneldelete = new MessageEmbed()
            .setTitle(`Configuré le statut`)
            .setDescription(
              "**Indiquer le statut à avoir pour obtenir le role soutien**"
            )
            .setColor(color)
            .setImage(
              "https://cdn.discordapp.com/attachments/904084986536276059/991547628657582120/standard_5.gif"
            );
          ...
          col.on("collect", async (i) => {
            if (i.values[0] == "Cancel") {
              ...
              menumsg.edit({
                embeds: [antichannel],
                components: [new MessageActionRow().addComponents([options])],
              });
              await i.deferUpdate();
            }
            if (i.values[0] == "active") {
              ...
              if (link == true) {
                message.channel
                  .send(`✅ |\`Un rôle \` est déjà setup`)
                  .then((msg) => {
                    setTimeout(() => msg.delete(), 10000);
                  })
                  .catch(console.error);
              } else {
                const oui = await message.channel.send(
                  "Quelle rôle doit être attribué ?"
                );
                let collected = message.channel
                  .awaitMessages({
                  ...
                  .then((collected) => {
                    ...
                    let role = coll.mentions.roles.first();
                    ...
                    if (
                      ...
                      return message.channel.send(
                        "Ce role ne peut pas etre défini en role de `soutien` car il possède des permissions dangereuses"
                      );
                    ...
                    message.channel
                      .send(
                        `✅ |\`Le module soutien \` a été activé avec succès, rôle soutien : **${role.name}**`
                      )
                      .then((msg) => {
              ...
              menumsg.edit({
                embeds: [MenuEmbed],
                components: [
                  new MessageActionRow().addComponents([menuoptions]),
                ],
              });
              await i.deferUpdate();
              ...
              if (link == true) {
                ...
                message.channel
                  .send(`❌ |\`Le rôle de soutien \` vient d'être reset`)
                  .then((msg) => {
                    setTimeout(() => msg.delete(), 10000);
                  })
                  .catch(console.error);
                ...
                message.channel
                  .send(`❌ |\`Le rôle de soutien \` est déjà reset`)
                  .then((msg) => {
                    setTimeout(() => msg.delete(), 10000);
                  })
                  .catch(console.error);
            ...
            //Statut
              ...
              menumsg.edit({
                embeds: [antichanneldelete],
                components: [
                  new MessageActionRow().addComponents([AntiChannelDelete]),
                ],
              });
              await i.deferUpdate();
            }
            if (i.values[0] == "activedel") {
              ...
              if (link == true) {
                message.channel
                  .send(`✅ |\`Le module de statut \` est déjà activé`)
                  .then((msg) => {
                    setTimeout(() => msg.delete(), 10000);
                  })
                  .catch(console.error);
              } else {
                const ez = await message.channel.send(
                  "Quelle doit être dans le statut?(*Les espaces ne seront pas comptés*)"
                );
                let collected = await message.channel
                  ...
                  .then((collected) => {
                    ...
                    message.channel
                      .send(
                        `✅ |\`Le statut à été set up \` avec comme statut ${status}`
                      )
                      .then((msg) => {
                        setTimeout(() => msg.delete(), 10000);
                      });
              ...
              if (link == true) {
                ...
                message.channel
                  .send(`❌ |\`Le statut \` vien d'être reset`)
                  .then((msg) => {
                    setTimeout(() => msg.delete(), 10000);
                  })
                  .catch(console.error);
            ...
            if (i.values[0] === "activemodule") {
              ...
              if (soutien === true) {
                return message.channel
                  .send("Le module de soutien est déjà activé")
                  .then((msg) => {
                ...
                db.set("support" + message.guild.id, true);
                return message.channel
                  .send("✅ |Le module de soutien vient d'être activé.")
                  .then((msg) => {
                    setTimeout(() => msg.delete(), 5000);
                  });
              }
              ...
              if (soutien == true) {
                db.set("support" + message.guild.id, null);
                return message.channel
                  .send("❌ |Le module de soutien vient d'être désactivé.")
                  .then((msg) => {
                    setTimeout(() => msg.delete(), 5000);
                  });
              } else
                return message.channel
                  .send("✅ |Le module de soutien est déjà désactivé.")
                  .then((msg) => {
                    setTimeout(() => msg.delete(), 5000);
                  });
            }
            ...
            if (i.values[0] === "Settings") {
              ...
              const paramètre = new MessageEmbed()
                ...
                .setColor(color)
                .setDescription(
                  `**Voici la configuration du soutien**\n\n**Rôle Soutien:** <@&${role}>\n**Statut à avoir**: __${statut}__`
                );
