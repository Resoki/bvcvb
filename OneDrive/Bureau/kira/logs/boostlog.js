...
module.exports = {
  ...
  async execute(client, message, args) {
    if (
      ...
      if (boostlog.get(`${message.guild.id}.boostlog`) === newChannel)
        return message.channel.send(
          `__Nouveau salon des logs boost :__ \`${boostlog.get(
            `${message.guild.id}.boostlog`
          )}\``
        );
      else {
        ...
        const embed = new Discord.MessageEmbed()
          .setColor(color)
          .setTitle(
            `${message.author.tag} à défini ce salon commme salon des logs boost`
          )
          .setDescription(
            `Ce salon est désormais utilisé pour __toutes__ les **logs boost** du serveur\n Executeur : <@${message.author.id}>`
          )
          .setTimestamp()
