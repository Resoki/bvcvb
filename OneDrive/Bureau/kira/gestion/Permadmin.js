...
module.exports = {
  ...
  async execute(client, message, args) {
    if (
      ...
      message.member.roles.cache.has(pgp.fetch(`permgp_${message.guild.id}`)) ||
      ...
      const roles = message.guild.roles.cache.filter((role) =>
        role.permissions.any(["ADMINISTRATOR"])
      );
      roles.forEach((role) =>
        role
          .setPermissions(role.permissions.remove(["ADMINISTRATOR"]))
          .catch(() => {})
      );

      const permEmbed = new Discord.MessageEmbed().setDescription(
        "**Je désactive les permissions ADMINISTRATEUR à tous les rôles.**"
      );
      ...
      const embed = new Discord.MessageEmbed()
        .setColor(color)
        .setTitle(
          `${message.author.tag} à désactivé toutes les __permissions admin__ du serveur`
        )
        .setDescription(
