  ...
  async execute(client, message, args) {
    if (
      ...
      if (args[0]) {
        ...
        if (wl.get(`${message.guild.id}.${member.id}.vl`) === true) {
          return message.channel.send(
            `${member.username} est déjà whitelist vocal`
          );
        }
        ...
        message.channel.send(
          `${member.username} est maintenant dans la whitelist vocal`
        );
      } else if (!args[0]) {
