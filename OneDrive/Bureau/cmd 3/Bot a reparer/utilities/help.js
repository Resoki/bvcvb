...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    if (img == null)
      img =
        "https://media.discordapp.net/attachments/1036241848580194334/1042484584069206086/kosix3.gif";
    ...
    if (args[0] === "settings") {
      if (
        ...
        message.channel
          .send({ embeds: [embed], components: [helpset] })
          .then(async (msg) => {
            ...
            collector.on("collect", async (c) => {
              ...
              if (value === "menu") {
                db.set(`help`, "help");
                c.reply({
                  content: `Votre help sera désormais affiché sous forme de **menu**`,
                  ephemeral: true,
                });
                ...
                db.set(`help`, "helpb");
                c.reply({
                  content: `Votre help sera désormais affiché avec des **boutons**`,
                  ephemeral: true,
                });
    ...
    if (args[0] === "msg") {
      ...
      const embed = new Discord.MessageEmbed()
        ...
        .setDescription(
          `Exemple de message simple: \`{MemberMention} nous a rejoint,  nous sommes maintenant {MemberCount} sur {Server}\``
        )
        .addFields(
          {
            name: "{MemberName}",
            value: "Le nom du membre concerné\n`Exemple: Funny`",
            inline: true,
          },
          {
            name: "{MemberMention}",
            value: `Mentionne le membre concerné\n\`Exemple:\` <@${message.author.id}>`,
            inline: true,
          },
          {
            name: "{MemberTag}",
            value: "Le nom et le # du membre concerné\n`Exemple: Funny#0666`",
            inline: true,
          }
        )
        .addFields(
          {
            name: "{MemberID}",
            value: `L'ID du membre concerné\n\`Exemple: ${message.author.id}\``,
            inline: true,
          },
          {
            name: "{MemberCount}",
            value: `Le nombre total de membres sur le serveurn\n\`Exemple: ${message.guild.memberCount}\``,
            inline: true,
          },
          {
            name: "{Server}",
            value: `Le nom du serveur\n\`Exemple: ${message.guild.name}\``,
            inline: true,
          }
        )
        .addFields(
          {
            name: "{ServerBoostsCount}",
            value: `Le nombre de boost du serveur\n\`Exemple: ${
          ...
          {
            name: "{ServerLevel}",
            value: `Le niveau actuel du serveur\n\`Exemple: ${
              premiumTier[message.guild.premiumTier]
            }\``,
            inline: true,
          },
          {
            name: "{VocalMembersCount}",
            value: `Le nombre total de membres en vocal sur le serveur\n\`Exemple: ${
        ...
        .addFields({
          name: "{OnlineMembersCount}",
          value: `Le nombre total de membres en ligne sur le serveur\n\`Exemple: ${
            message.guild.presences.cache.filter(
    ...
    if (args[0] === "all") {
      const imgperm =
        "https://cdn.discordapp.com/attachments/904084986536276059/1004761057056805024/standard_9.gif";
      ...
      const help = new Discord.MessageEmbed()
        .setTitle(`ℹ️・Panel d'aide Permissions ${client.user.username}`)
        .setDescription(
          `・Ce bot appartient à <@${config.app.owners}>`
        )
        .setImage(imgperm)
        .setColor(color)
...
\`\`\`
    ...
    **\`${pf}setname\`**
    ...
    if (helpm == "help") {
      ...
      const Help = new Discord.MessageEmbed()
        .setTitle(
          `ℹ️ ・Panel d'aide de ${client.user.username}`
        )
        .setDescription(
          `・Ce bot appartient à <@${config.app.owners}>`
        )
...
Change le nom du bot
      ...
      message.channel
        .send({ embeds: [Help], components: [row] })
        .then(async (msg) => {
          const collector = message.channel.createMessageComponentCollector({
    ...
    if (db.get(`help`) === "helpb") {
      const Help = new Discord.MessageEmbed()
        .setTitle(`ℹ️・Panel d'aide de ${client.user.username}`)
        .setDescription(
          `・Ce bot appartient à <@${config.app.owners}>`
        )
        .setImage(img)
