  ...
  async execute(client, oldMember, newMember) {
    ...
    if (oldMember.roles.cache.size < newMember.roles.cache.size) {
        ...
        if (
          ...
          if (logsAdmin) {
            const embed = new Discord.MessageEmbed()
              .setAuthor({
                name: `Permission Admin détectée [Danger]`,
              ...
              .setFooter({ text: `⚠️` })
              .setColor(color);
            const msg = await logsAdmin.send({
              content: `${pingrole}`,
              embeds: [embed],
            });
          }
      ...
      if (aa.get(`config.${oldMember.guild.id}.antiadmin`) === true) {
        if (
          owner.get(`owners.${executor.id}`) ||
          (config.app.owners === executor.id) === true ||
          (client.user.id === executor.id) === true
        )
          return;
        ...
        oldMember.guild.members
          ...
          .roles.cache.forEach((role) => {
            if (role.name !== "@everyone") {
              ...
              punish.get(`sanction_${oldMember.guild.id}`) === "derank"
            ) {
              oldMember.guild.members
              ...
              punish.get(`sanction_${oldMember.guild.id}`) === "kick"
            ) {
            ...
            const embed = new Discord.MessageEmbed()
              .setDescription(
                `<@${audit.executor.id}> a tenté d'ajouté un role possédant une \`perm admin\` a <@${newMember.id}>, ils ont été sanctionné`
              )
              .setTimestamp();
            client.channels.cache
              .get(rlog.fetch(`${oldMember.guild.id}.raidlog`))
              .send({ embeds: [embed] })
              .catch(console.error);
          });
      }
    ...
    if (db.get(`banping_${oldMember.guild.id}`) == true) {
      ...
      if (
        ...
        if (logsAdmin) {
          const embed = new Discord.MessageEmbed()
            .setAuthor({
              name: `Permission Ban détectée [Danger]`,
              iconURL: newMember.user.displayAvatarURL({ dynamic: true }),
            })
    ...
    if (db.get(`roleping_${oldMember.guild.id}`) == true) {
      ...
      if (
        ...
        if (logsAdmin) {
          const embed = new Discord.MessageEmbed()
            .setAuthor({
              name: `Permission role détectée [Danger]`,
              iconURL: newMember.user.displayAvatarURL({ dynamic: true }),
            })
