  ...
  async execute(client, interaction, message) {
    ...
    if (interaction.customId === "ticket") {
      if (interaction.values[0] == "delete") {
        interaction.reply({
          content: `Veuilez confirmer la fermeture de votre ticket`,
          ephemeral: true,
        });
        ...
        const embed = new Discord.MessageEmbed()
          ...
          .setDescription(
            `<@${interaction.member.id}> Etes vous sur de vouloir fermé ce ticket ?`
          )
      ...
      if (interaction.values[0] == "ticketdelete") {
        ...
        hastebin(
          ...
          .then((haste) => {
            ...
            client.channels.cache.get(config.app.channelLogs).send({
    ...
    if (interaction.customId === "ticketdelete") {
      if (interaction.values[0] == "transcript") {
        ...
        const msgd = await interaction.channel.send({
          content:
            "<a:warn:986267997717020712> Récupération des messages, cela peut prendre un certain temps...",
        });
