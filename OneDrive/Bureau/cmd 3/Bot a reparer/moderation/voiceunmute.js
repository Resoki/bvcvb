    ...
    if (
      ...
      if (!muteUser.voice.serverMute) {
        return message.channel.send(
          "Le membre n'est pas dans un salon vocal ou est déjà unmute vocal."
        );
      }
      ...
      try {
        ...
        console.error(err);
        message.reply(
          "Je n'ai pas pu désactiver le son de cet utilisateur, veuillez vérifier mes permissions et réessayer.\n" +
            err
        );
      }
      ...
      try {
        muteUser.user.send(
          `Vous avez été **unmute** sur **${message.guild.name}**, Raison: **${
            muteReason || "Aucune"
          }**.`
        );
      } catch (err) {
      ...
      message.channel.send(
        `**${
          muteUser.user.tag
        }** a été unmute avec succès sur le serveur. Raison: **${
          muteReason || "Aucun"
        }**. `
      );
      ...
      const embed = new Discord.MessageEmbed()
        .setColor(color)
        .setDescription(
          `<@${message.author.id}> a \`voiceunmute\` ${muteUser}\nRaison: ${muteReason}`
        )
        .setTimestamp()
        .setFooter({ text: `📚` });
