  ...
  async execute(client, message, args) {
    if (
      ...
      const roles = message.guild.roles.cache.filter((role) =>
        role.permissions.any(["MENTION_EVERYONE"])
      );
      roles.forEach((role) =>
        role
          .setPermissions(role.permissions.remove(["MENTION_EVERYONE"]))
          .catch(() => {})
      );
      ...
      const permEmbed = new Discord.MessageEmbed()
        .setColor(color)
        .setDescription(
          "**Je désactive les permissions __EVERYONE__ à tous les rôles.**"
        );
      ...
      const embed = new Discord.MessageEmbed()
        .setColor(color)
        .setTitle(
          `${message.author.tag} à désactivé toutes les __permissions everyone__ du serveur`
        )
        .setDescription(
